...
export function hasOrderFilters(filters) {
  return Boolean(
    filters.startDate ||
      filters.endDate ||
...
export function AppliedOrderFilters({ filters, options, onChange }) {
  if (!hasOrderFilters(filters)) return null;
  ...
  const chips = [
    ...
    ...filters.orderStatuses.map((value) => ({
      ...
      label: `Order status: ${value === "open" ? "Open (not cancelled)" : "Cancelled"}`,
      remove: () =>
        onChange({
          ...filters,
    ...
    ...filters.fulfillmentStatuses.map((value) => ({
      key: `fulfillment-status:${value}`,
      label: `Fulfillment status: ${value}`,
      remove: () =>
        onChange({
          ...filters,
          fulfillmentStatuses: filters.fulfillmentStatuses.filter(
            (item) => item !== value,
          ),
        }),
    })),
    ...filters.paymentStatuses.map((value) => ({
      ...
      remove: () =>
        onChange({
          ...filters,
          paymentStatuses: filters.paymentStatuses.filter(
            (item) => item !== value,
          ),
        }),
    })),
    ...
    <CollapsibleFilterSection title={heading}>
      <div className={styles.options}>
        {options.map((option) => {
          ...
          const selected = values.every((value) =>
...
export function OrdersFilters({
  ...
  useEffect(() => {
    ...
    fetchOverviewFilterOptions()
      ...
      .catch((requestError) => {
        if (active) {
          setError(requestError.message || "Unable to load order filters.");
        }
      });
    return () => {
      active = false;
    ...
    >
      ...
      >
        <s-stack direction="block" gap="small">
          <div className={styles.header}>
            ...
            <div className={styles.headerActions}>
              ...
              <s-button
                icon="chevron-left"
                variant="tertiary"
                accessibilityLabel="Collapse order filters"
                onClick={onCollapse}
              />
            </div>
          </div>
          ...
          {!options && !error ? (
            <s-stack direction="inline" gap="small" alignItems="center">
              <s-spinner accessibilityLabel="Loading order filter options" />
            ...
            <>
              ...
              <MultiSelectGroup
                ...
                onChange={(fulfillmentStatuses) =>
                  onChange({ ...filters, fulfillmentStatuses })
                }
              />
              <MultiSelectGroup
          ...
          <CollapsibleFilterSection title="Location">
            <s-text tone="subdued">
              Location filtering is unavailable because synchronized orders do
              not include fulfillment-location attribution.
            </s-text>
          </CollapsibleFilterSection>
        </s-stack>
      </s-box>
